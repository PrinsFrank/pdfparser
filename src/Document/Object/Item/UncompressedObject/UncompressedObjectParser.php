...
class UncompressedObjectParser {
    public static function parseObject(CrossReferenceEntryInUseObject $crossReferenceEntry, int $objectNumber, Document $document): UncompressedObject {
        $endObj = $document->stream->firstPos(Marker::END_OBJ, $crossReferenceEntry->byteOffsetInDecodedStream, $document->stream->getSizeInBytes()) ?? throw new ParseFailureException('Unable to locate end of object');
        $startObj = $document->stream->firstPos(Marker::OBJ, $crossReferenceEntry->byteOffsetInDecodedStream, $endObj) ?? throw new ParseFailureException('Unable to locate start of object');
