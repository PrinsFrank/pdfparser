...
class TextParserTest extends TestCase {
    public function testParseText(): void {
        static::assertEquals(
            new ContentStream(
                (new TextObject())
                    ->addCommand(new ContentStreamCommand(TextStateOperator::FONT_SIZE, '/F1 24'))
                    ->addCommand(new ContentStreamCommand(TextPositioningOperator::MOVE_OFFSET, '100 100'))
                    ->addCommand(new ContentStreamCommand(TextShowingOperator::SHOW, '( Hello World )'))
            ),
